             ...
             <div class="iq-card-body">
                <div class="stepwizard mt-4">
                   <div class="stepwizard-row setup-panel">
                      ...
                      <div id="document" class="wizard-step active">
                ...
                <form class="form" id="yuk-bildir">
                         ...
                         <div class="col-md-12 p-0">
                                ...
                                <div class="col-md-3 form-group">
                                    ...
                                    <select class="form-control" id="sofor_1_id"name="sofor_1_id" >
                                        ...
                                        @php( $sofors = \App\SoforModel::where('company_id',Auth::user()->company_id)->get())
                                         ...
                                         <option {{ selected($SeferModel->driver_1_id,$item->id) }} value="{{ $item->id }}">{{ $item->name }} {{ $item->surname }}</option>
                                         @endforeach
                                     </select>
                                </div>
                           ...
                           <div class="col-md-3 form-group">
                                    ...
                                    <select class="form-control" id="sofor_2_id"name="sofor_2_id" >
                                        ...
                                        @php( $sofors = \App\SoforModel::where('company_id',Auth::user()->company_id)->get())
                                         @foreach ($sofors as $item)
                                         <option {{ selected($SeferModel->driver_2_id,$item->id) }} value="{{ $item->id }}">{{ $item->name }} {{ $item->surname }}</option>
                                         @endforeach
                                     </select>
                                </div>
                             <div class="col-md-3 form-group">
                                      ...
                                      <select class="form-control" id="plaka"name="plaka" >
                                          ...
                                          @php( $PlakaModel = \App\PlakaModel::where('company_id',Auth::user()->company_id)->get())
                                           ...
                                           <option {{ selected($SeferModel->plate_1_id,$item->id) }} value="{{ $item->id }}">{{ $item->plate }}</option>
                                           @endforeach
                                       </select>
                                  </div>
                             ...
                             <div class="col-md-3 form-group">
                                      ...
                                      <select class="form-control" id="yedek_plaka"name="yedek_plaka" >
                                          ...
                                          @php( $PlakaModel = \App\PlakaModel::where('company_id',Auth::user()->company_id)->get())
                                           @foreach ($PlakaModel as $item)
                                           <option {{ selected($SeferModel->plate_2_id,$item->id) }} value="{{ $item->id }}">{{ $item->plate }}</option>
                                           @endforeach
                                       </select>
                                  </div>
                                  <div class="col-md-4 form-group">
                                       <label for="sefer_baslangic_tarihi">Sefer Başlangıç Tarihi</label>
                                       <input type="date" value="{{$SeferModel->start_date}}" class="form-control" id="sefer_baslangic_tarihi"name="sefer_baslangic_tarihi" value="{{ date('Y-m-d') }}">
                                   </div>
                                   <div class="col-md-2 form-group">
                                        <label for="sefer_baslangic_saati">Sefer Başlangıç Saati</label>
                                        <input type="time" value="{{$SeferModel->start_time}}" class="form-control" id="sefer_baslangic_saati"name="sefer_baslangic_saati" value="{{ date('H:i') }}">
                                    </div>
                                    <div class="col-md-4 form-group">
                                         <label for="sefer_bitis_tarihi">Sefer Bitiş Tarihi</label>
                                         <input type="date" value="{{$SeferModel->end_date}}" class="form-control" id="sefer_bitis_tarihi"name="sefer_bitis_tarihi" value="{{ date('Y-m-d') }}">
                                     </div>
                                     <div class="col-md-2 form-group">
                                          <label for="sefer_bitis_saati">Sefer Bitiş Saati</label>
                                          <input type="time" value="{{$SeferModel->end_time}}" class="form-control" id="sefer_bitis_saati"name="sefer_bitis_saati" value="{{ date('H:i') }}">
                                      </div>
                                      <div class="col-md-12 form-group">
                                        <label for="aciklama">Size Özel Açıklama 1 </label>
                                        <textarea class="form-control" id="aciklama" name="aciklama" rows="10"> {!! $SeferModel->note !!}</textarea>
                                       </div>
                   ...
                   <div class="row setup-content" id="document-detail">
                      ...
                      <div class="col-sm-12 divSide">
                        @foreach($SeferModel->yukler as $key=>$yuk)
                        <div class="col-md-12 p-0">
                            <h3 class="mb-4">#{{$key+1}} Yük Detaylarınız</h3>
                            <div class="row">

                                <div class="col-md-6 form-group">
                                    <label for="company_id">Yük Türü</label>

                                <select name="yuk[{{$key}}][yukCinsId]" title="Yük Türünü Seçiniz" class="form-control yukCinsId" >

                                    @foreach ($yuk_tur_listesi as $item)
                                    <option {{selected($yuk->yuk_cins_id, $item->id)}} value="{{ $item->id }}"  key="{{ $item->type }}" >{{ $item->name }}</option>
                                    @endforeach
                                                                                                        </select>
                                                                                                    </div>
                                        <div class="col-md-2 form-group">
                                            <label for="company_id">Yük Miktarı</label>
                                             <input placeholder="örn: 12.5" required="" value="{{$yuk->yuk_adet}}" title="Taşınan yük miktarı" type="text" class="form-control txtQty" name="yuk[{{$key}}][yukMiktari]">
                                        ...
                                        <div class="col-md-4 form-group">
                                            ...
                                            <select name="yuk[{{$key}}][yukBirimi]" title="Yük birimini seçin" required="" class="form-control select2 yukBirimi "  >

                                                @foreach ($yuk_birim_listesi as $item)
                                                <option {{selected($yuk->yuk_birim, $item->id)}} value="{{ $item->id }}" style="display: none" keyData="{{ $item->type }}" >{{ $item->name }}</option>
                                                @endforeach
                                            </select>
                                ...
                                <select name="yuk[{{$key}}][un_turu]" title="Yük Türünü Seçiniz" class="form-control  unTuru" >

                                    @foreach ($yuk_tehlikeli_madde_un_listesi as $item)
                                    <option {{selected($yuk->yuk_unId, $item->id)}} value="{{ $item->id }}" >{{ $item->name }}</option>
                                    @endforeach
                                                                                                        </select>
                                                                                                    </div>
                                ...
                                <select name="yuk[{{$key}}][tehlikeli_madde_tasima_sekli]" title="Yük Türünü Seçiniz" class="form-control " >

                                    @foreach ($yuk_tehlikeli_madde_tasima_sekli as $item)
                                    <option {{selected($yuk->yuk_tehlikeli_madde, $item->id)}}  value="{{ $item->id }}" >{{ $item->name }}</option>
                                    @endforeach
                                                                                                        </select>
                                        ...
                                        <div class="col-md-4 form-group tehlikeli-div" style="display: none">
                                            ...
                                            <select name="yuk[{{$key}}][muaf_turu]" title="Muafiyet Türü   seçin" required="" class="form-control select2  "  >
                                                @foreach ($yuk_tehlikeli_madde_muaf_turleri as $item)
                                                <option {{selected($yuk->yuk_muaf_tur, $item->id)}} value="{{ $item->id }}"  >{{ $item->name }}</option>
                                                @endforeach
                                            </select>
                                ...
                                <div class="col-md-6 form-group">
                                    <label for="company_id">Gönderen Firma</label>
                                        ...
                                        <select class="form-control" id="yuk[{{$key}}][from_company_id]"name="yuk[{{$key}}][from_company_id]" >
                                          @php( $companys = \App\CompanyModel::where('company_user_id',Auth::user()->company_id)->get())
                                            @foreach ($companys as $item)
                                            <option {{selected($yuk->company_id, $item->id)}} value="{{ $item->id }}">{{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                 </div>
                                 <div class="col-md-6 form-group">
                                    <label for="company_id">Alıcı Firma</label>
                                        ...
                                        <select class="form-control" id="yuk[{{$key}}][to_company_id]"name="yuk[{{$key}}][to_company_id]" >
                                          @php( $companys = \App\CompanyModel::where('company_user_id',Auth::user()->company_id)->get())
                                            @foreach ($companys as $item)
                                            <option {{selected($yuk->alici_company_id, $item->id)}}  value="{{ $item->id }}">{{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                 </div>
                                <div class="col-md-3 form-group">
                                    <label for="uyruk">Yükleme Ülke </label>
                                    <select class="form-control" id="yuk[{{$key}}][yukleme_ulke]"name="yuk[{{$key}}][yukleme_ulke]" >
                                       @php( $ulke = \DB::table('ulkeler')->get())
                                         @foreach ($ulke as $item)
                                         <option {{selected($yuk->yuk_ulke_id, $item->id)}} value="{{ $item->id }}">{{ $item->country }} </option>
                                         @endforeach
                                     </select>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="uyruk">Yükleme İl İlçe </label>
                                    <select class="form-control" id="yuk[{{$key}}][yukleme_il_ilce_kod]"name="yuk[{{$key}}][yukleme_il_ilce_kod]" >
                                        <option disabled="">İl İlçe Seçiniz</option>
                                        @php( $ililce = \DB::table('il_ilce_kod')->get())
                                         @foreach ($ililce as $item)
                                         <option {{selected($yuk->yuk_il_mernis_id, $item->id)}} value="{{ $item->id }}">{{ $item->il }}-{{ $item->ilce }} </option>
                                         @endforeach
                                     </select>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="uyruk">Boşaltma Ülke </label>
                                    <select class="form-control" id="yuk[{{$key}}][bosaltma_ulke]"name="yuk[{{$key}}][bosaltma_ulke]" >
                                        @php( $ulke = \DB::table('ulkeler')->get())
                                         @foreach ($ulke as $item)
                                         <option {{selected($yuk->yuk_bosalt_ulke_id, $item->id)}} value="{{ $item->id }}">{{ $item->country }} </option>
                                         @endforeach
                                     </select>
                                </div>
                                <div class="col-md-3 form-group">
                                    <label for="uyruk">Boşaltma İl İlçe </label>
                                    <select class="form-control" id="yuk[{{$key}}][bosaltma_il_ilce_kod]"name="yuk[{{$key}}][bosaltma_il_ilce_kod]" >
                                        <option disabled="">İl İlçe Seçiniz</option>
                                        @php( $ililce = \DB::table('il_ilce_kod')->get())
                                         @foreach ($ililce as $item)
                                         <option {{selected($yuk->yuk_bosalt_il_mernis_id, $item->id)}} value="{{ $item->id }}">{{ $item->il }}-{{ $item->ilce }} </option>
                                         @endforeach
                                     </select>
                                </div>
                                  <div class="col-md-4 form-group">
                                       <label for="exampleInputdate">Yükleme Tarih </label>
                                       <input type="date"value="{{$yuk->yuk_yukleme_date}}" class="form-control" id="yuk[{{$key}}][yukleme_tarih]"  name="yuk[{{$key}}][yukleme_tarih]" value="{{ date('Y-m-d') }}">
                                   </div>
                                   <div class="col-md-2 form-group">
                                        <label for="exampleInputdate">Yükleme Saati</label>
                                        <input type="time"value="{{$yuk->yuk_yukleme_time}}" class="form-control" id="yuk[{{$key}}][yukleme_saat]"  name="yuk[{{$key}}][yukleme_saat]" value="{{ date('H:i') }}">
                                    </div>
                                    <div class="col-md-4 form-group">
                                         <label for="exampleInputdate">Boşaltma  Tarihi</label>
                                         <input type="date"value="{{$yuk->yuk_bosaltma_date}}" class="form-control" id="yuk[{{$key}}][bosaltma_tarihi]" name="yuk[{{$key}}][bosaltma_tarihi]" value="{{ date('Y-m-d') }}">
                                     </div>
                                     <div class="col-md-2 form-group">
                                          <label for="exampleInputdate">Boşaltma  Saati</label>
                                          <input type="time"value="{{$yuk->yuk_bosaltma_time}}" class="form-control" id="yuk[{{$key}}][bosaltma_saati]" name="yuk[{{$key}}][bosaltma_saati]" value="{{ date('H:i') }}">
                                      </div>
...
@section('script')
    <script>
        ...
        var i=parseInt({{count($SeferModel->yukler)}});
        ...
        var htmlEl =  '<div class="row">'+
                        '<div class="col-md-12  form-group">'+
                            '<h3 class="mb-4">'+(i)+'. Yük Detaylarınızı Girin:</h3>'+
